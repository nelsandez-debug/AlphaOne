...
export function RiskFlagRow({ flag, editable }: { flag: Flag; editable: boolean }) {
  ...
  return (
    <div className="flex items-center justify-between glass p-4">
      <div>
        <Link href={`/suppliers/${flag.supplier.id}`} className="text-sm font-medium text-slate-900 hover:text-accent-700">{flag.supplier.name}</Link>
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        {editable && (
          <button type="button" onClick={toggle} className="text-xs font-medium text-accent-700 hover:underline">
