...
export function CreateIntakeForm() {
  ...
  return (
    <form onSubmit={submit} className="flex flex-col gap-2 rounded-lg border border-slate-200 bg-white p-3 w-full max-w-md">
      <input
        autoFocus
        value={title}
      ...
      <div className="flex gap-2">
        <select value={type} onChange={(e) => setType(e.target.value as typeof type)} className="rounded border border-slate-200 px-2 py-1 text-xs flex-1">
        ...
        <input
          ...
          className="rounded border border-slate-200 px-2 py-1 text-xs flex-1 outline-none focus:border-[#2563EB]"
        />
      </div>
      <textarea
        value={description}
