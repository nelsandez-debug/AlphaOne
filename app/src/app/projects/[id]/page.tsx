    ...
    <div className="mx-auto w-full max-w-4xl space-y-6 p-8">
      <div>
        <div className="flex items-center gap-2 text-xs font-medium">
          {project.supplier && (
            <Link href={`/suppliers/${project.supplier.id}`} className="text-accent-700 hover:underline">{project.supplier.name}</Link>
          )}
          {project.sourcingEvent && (
            <>
              {project.supplier && <span className="text-slate-300">·</span>}
              <Link href={`/sourcing/${project.sourcingEvent.id}`} className="text-accent-700 hover:underline">{project.sourcingEvent.title}</Link>
            </>
          )}
        </div>
        ...
        <RelationshipCard icon={ShoppingCart} title="Purchase Orders" count={project.purchaseOrders.length} href={`/purchase-orders?projectId=${id}`}>
          <div className="space-y-2">
            {project.purchaseOrders.map((po) => (
              <Link key={po.id} href={`/purchase-orders/${po.id}`} className="block rounded-lg bg-neutral-100 border border-neutral-200 px-3 py-2 hover:border-accent-500">
        ...
        <RelationshipCard icon={Receipt} title="Invoices" count={project.invoices.length} href={`/invoices?projectId=${id}`}>
          <div className="space-y-2">
            {project.invoices.map((inv) => (
              <Link key={inv.id} href={`/invoices/${inv.id}`} className="block rounded-lg bg-neutral-100 border border-neutral-200 px-3 py-2 hover:border-accent-500">
